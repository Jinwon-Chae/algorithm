...
func rotateAfterInsert[K constraints.Ordered, V any](n *TreeNode[K, V], key K) *TreeNode[K, V] {
	...
	if balance > 1 && key < n.Left.Key { // type 1 - LL
		return rightRotate(n)
	}

	if balance < -1 && key >= n.Right.Key { // type 1 - RR
		return leftRotate(n)
	}

	if balance > 1 && key >= n.Left.Key { // type 2 - LR
		n.Left = leftRotate(n.Left)
		return rightRotate(n)
	}

	if balance < -1 && key < n.Right.Key { // type 1 - RL
...
func rotateDelete[K constraints.Ordered, V any](n *TreeNode[K, V]) *TreeNode[K, V] {
	...
	if balance > 1 && n.Left.getBalance() >= 0 {
		// LL - type1
		return rightRotate(n)
	} else if balance > 1 && n.Left.getBalance() < 0 { // LR - type2
		n.Left = leftRotate(n.Left)
		return rightRotate(n)
	}

	if balance < -1 && n.Right.getBalance() <= 0 { // RR - type1
		return leftRotate(n)
	}

	if balance < -1 && n.Right.getBalance() > 0 { // RL - type2
		n.Right = rightRotate(n.Right)
		return leftRotate(n)
	}
