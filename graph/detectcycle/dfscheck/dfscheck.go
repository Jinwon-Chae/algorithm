...
func DetectLoop[T any](head *Node[T]) bool {
	...
	return internalDetectionLoop(head, ancestors)
}

func internalDetectionLoop[T any](node *Node[T], ancestors map[*Node[T]]bool) bool {
	ancestors[node] = true
	for _, l := range node.Links {
		...
		if internalDetectionLoop[T](l, ancestors) {
			return true
		}
